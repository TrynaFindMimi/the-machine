...
| Dominio | `core/` | `perceptron.py`, `results.py`, `handedness.py`, `gestures.py`, `fingers.py`, `playlist.py` | `config`, `common`, `numpy` | Lógica pura sin cv2/pygame. `handedness.py` aísla corrección flip `get_handedness`/`normalize_handedness`; `results.py` solo `get_gesture`/`to_pixel_points`/`count_hands`; `fingers.py` features binarias por dedo (coseno del angulo de flexion `MCP→PIP→TIP`, umbrales por dedo `EXTEND_THRESHOLDS`) + `index_thumb_distance`/`pinky_thumb_distance`/`hand_scale`; `playlist.py` `scan_sagas` |
| Controladores | `controllers/` | `hand.py`, `gestures.py`, `volume.py` | `config`, `core`, `numpy` | `FingerRNN` x5 + `HandController` (count/action/train/save/load); `MusicGestureController` (ventana, agreement, settle 0.45s, rate limit 1.5s, sin re-petición, acción pendiente); `VolumeController` (Perceptron 2-f, commit por meñique↔pulgar, re-arm) |
