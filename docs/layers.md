...
| Controladores | `controllers/` | `hand.py`, `thumb.py`, `gestures.py`, `volume.py` | `config`, `core`, `numpy` | `FingerRNN` x4 + `HandController` (count con mayoría de pulgar / action / train / save / load); `ThumbPerceptron` (3 features de palma, `models/thumb.npz`); `MusicGestureController` (ventana, agreement, settle 0.45s, rate limit 1.5s, sin re-petición, acción pendiente); `VolumeController` (Perceptron 2-f, commit por meñique↔pulgar, re-arm) |
