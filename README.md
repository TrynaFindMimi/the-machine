...
| Mano abierta (>=4 dedos) | Reproducir |
...
app/runner → consume_pending_action → player.play()/pause()
```
...
**music** — Reproductor de EPIC por sagas (folders `NN Title.mp3`). UI con caja negra (`_put_text_box`): SAGA, SONG, TRACK, STATE, FINGERS (LSTM) y ACTION. Tags negros estilo `position`. Solo cabeza la mano derecha (`core/handedness.get_handedness`).
...
the-machine/
  ...
  core/
    fingers.py       # features_from_landmarks (5 binarias + hand-sign)
  ...
  models/
    hand_landmarker.task, gesture_recognizer.task, finger_0..4.npz
  music/
